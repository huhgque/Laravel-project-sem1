      ...
      >
        <nav aria-label="breadcrumb" class="mt-2">
          <ol class="breadcrumb mb-0 p-0">
            <li class="breadcrumb-item"><a href="{{route('slide.index')}}">Home</a></li>
            <li class="breadcrumb-item active" aria-current="page">
            Slide List
            </li>
          </ol>
        </nav>
        <a href="{{Route('slide.create')}}" class="btn btn-danger text-white ms-3 d-none d-md-block">
          Add new
        </a>
      </div>
    </div>
              ...
              <table id="file_export" class="table table-bordered">
                ...
                @foreach($slide as $value)
                  <tr>
                    ...
                    <td>{{$loop->index+1}}</td>
                    <td><img src="{{url('uploads')}}/{{$value->slide_image}}" alt="" width="100px"></td>
                    <td>{{$value->link_to}}</td>
                    <td>
                      <div class="btn-group">
                        ...
                        <div class="dropdown-menu">
                          ...
                          <a class="dropdown-item" href="{{Route('slide.edit',$value->id)}}">
                            <i data-feather="edit-2" class="feather-sm me-2"></i>
                            Edit
                          </a>
                          <form class="dropdown-item" style="cursor: pointer;" action="{{Route('slide.destroy',$value->id)}}" method="POST">
                            @csrf
                            @method('DELETE')
